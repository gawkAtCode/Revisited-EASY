...
def get_fasttext_aligned_vectors(words, device, lang):
    embs = {}
    with open(osp.join("aligned_vectors", 'wiki.{}.align.vec'.format(lang)), 'r', encoding="utf-8") as f:
...
def get_name_feature_map(sents, embedding_loader=None, device='cpu',
                         batch_size=4096, use_fasttext=False, lang=None,
                         **kwargs):
    word_id_map = {}
    entity2word = []
...
def cpm_embedding(ent2word, words, cpm_types, models=('en', 'fr')):
    word_vec = [get_fasttext_aligned_vectors(words, 'cpu', lang) for lang in models]
