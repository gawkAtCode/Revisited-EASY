...
def iterative_completion(src2trg: Tensor, trg2src: Tensor, eis: Tuple[Tensor, Tensor], lens: Tuple[int, int],
                         values: Optional[Tuple[Tensor, Tensor]] = None, th=0.15, strategy="SRS") -> \
        Tuple[Tensor, Tensor]:
    ...
    with torch.no_grad():
        ...
        ei_src, ei_trg = eis
        # print("get ei_src")
        ei_src = ind2sparse(ei_src, src_len, strategy=strategy)
        # print("get ei_trg")
        ei_trg = ind2sparse(ei_trg, trg_len, strategy=strategy)
        # print("get trans")
        trans = ind2sparse_original(trans, src_len, trg_len)
        # print("trans:",trans)
...
def graph_matching_distance(gs, gt, perm, sparse=True, argmax=True, eps=0.01):
    ...
    if sparse:
        ...
        perm = ind2sparse_original(xy, gs.size(0), gt.size(0))
        #print("perm:",perm)
        indices, values=perm._indices(),perm._values()
        gsP = spspmm(gs, perm)
