...
def token_level_similarity(src_w2e: Tensor, trg_w2e: Tensor, src_word_x: Tensor, trg_word_x: Tensor, sparse_k=1,
    ...
    # import faiss
    # print(src_word_x.shape,"IVF1024,PQ"+str(int(src_word_x.shape[1]))+"x4fsr,RFlat")
    # index=faiss.index_factory(src_word_x.shape[1], "IVF1024,PQ"+str(int(src_word_x.shape[1]))+"x4fsr,RFlat")
    # index.train(src_word_x)
    # index.add(src_word_x)
    # k=1 if sparse_k==1 else 10
    # D,I=index.search(trg_word_x, k+1)
    # indices, values=[[],[]],[]
    # for i in range(D.shape[0]):
        # for j in range(1,k+1):
            # indices[0].append(I[i][j])
            # indices[1].append(i)
            # values.append(D[i][j])
    # sim = torch.sparse.FloatTensor(torch.LongTensor(indices), torch.DoubleTensor(values), (src_word_x.shape[0],trg_word_x.shape[0]))
    # del index
