...
def dict_values_to_tensor(d: {}, device="cpu"):
...
def ind2sparse_original(indices: Tensor, size, size2=None, dtype=torch.float, values=None):
...
def ind2sparse_for_perm(indices: Tensor, size, size2=None, dtype=torch.float, values=None,num_it=20,curr_it=19):
    ...
    if values is None:
        ...
        if size[0]==size[1]:
            ...
            words=[str(i) for i in range(size[0])]
            name_words=["" for i in range(size[0])]
            edge1,edge2=[],[]
            for i in range(len(indices[0])):
                a,b=int(indices[0][i].item()),int(indices[1][i].item())
                edges[a].add(b)
            for i in range(len(indices[0])):
                a,b=int(indices[0][i].item()),int(indices[1][i].item())
                if a not in edges[b]:
            ...
            for a,b in edges.items():
                name_words[a]=" ".join([str(it) for it in b])
            tfidf=get_tf_idf(words, name_words, None)
            tfidf_sum=[np.sum(tfidf[i]) for i in range(tfidf.shape[0])]
            indices=torch.cat([indices,torch.Tensor([edge1,edge2])],dim=1)
            values = torch.ones([indices.size(1)], device=device, dtype=dtype)
            for i in range(len(indices[0])):
                a,b=int(indices[0][i].item()),int(indices[1][i].item())
                values[i]=1#(tfidf[a,b]/tfidf_sum[a])*len(edges[a])
            ...
            # G = nx.DiGraph()
            # G.add_nodes_from(list(edges.keys()))
            # for i in range(len(indices[0])):
                # a,b=int(indices[0][i].item()),int(indices[1][i].item())
                # G.add_edge(a, b)
            ...
            # for i in range(len(indices[0])):
                # a,b=int(indices[0][i].item()),int(indices[1][i].item())
                # values[i]=float(pr[b]/sum(w[a]))*len(w[a])
            # del G
            # print("pagerank end")
            ...
            # print("simrank start")
            # G = nx.DiGraph()
            # print("nodes:",len(list(edges.keys())))
            # print("edges:",len(indices[0]))
            # G.add_nodes_from(list(edges.keys()))
            # for i in range(len(indices[0])):
                # a,b=int(indices[0][i].item()),int(indices[1][i].item())
                # G.add_edge(a, b)
                # #G.add_edge(b, a)
            # sim=nx.simrank_similarity(G,max_iterations=1,tolerance=0.01)
            # sim=np.array([[sim[u][v] for v in G] for u in G])
            # values = torch.ones([indices.size(1)], device=device, dtype=dtype)
            # for i in range(len(indices[0])):
                # a,b=int(indices[0][i].item()),int(indices[1][i].item())
                # values[i]=sim[a,b]*len(edges[a])
            # del G
            # print("simrank end")
...
def ind2sparse(indices: Tensor, size, size2=None, dtype=torch.float, values=None, strategy="unknown"):
    if strategy=="unknown":
    ...
    if values is None:
        ...
        if size[0]==size[1]:
            if strategy=="ASRS-TFIDF":
                ...
                for i in range(len(indices[0])):
                    a,b=int(indices[0][i].item()),int(indices[1][i].item())
                    values[i]=(tfidf[a,b]/tfidf_sum[a])*len(edges[a])
            elif strategy=="ASRS-PageRank":
                edges=defaultdict(set)
                edge1,edge2=[],[]
                for i in range(len(indices[0])):
                    a,b=int(indices[0][i].item()),int(indices[1][i].item())
...
def rebuild_with_indices(sp: Tensor, strategy="unknown"):
    sp = sp.coalesce()
    return ind2sparse(sp._indices(), sp.size(0), sp.size(1), strategy=strategy).coalesce()
...
def to_torch_sparse(matrix, dtype=float, device="cpu"):
    matrix = matrix.tocoo()
...
def random_split(y: Tensor, total=15000, cnt_test=9000, cnt_train=4500, dim=1, device='cpu'):
...
def topk2sparse_mat(val0, ind0, size, dim=0, device: torch.device = 'cpu'):
